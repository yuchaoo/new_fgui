...
namespace fgui {
	...
	std::string UIConfig::defaultFont = "k1";
	...
	cocos2d::Color4F UIConfig::modalLayerColor = cocos2d::Color4F(0, 0, 0, 0.4f);
	...
	void UIConfig::registerFont(const std::string& aliasName, const std::string& realName)
	{
		...
		bool tmp = cocos2d::FileUtils::getInstance()->isPopupNotify();
		cocos2d::FileUtils::getInstance()->setPopupNotify(false);
		fi.ttf = cocos2d::FileUtils::getInstance()->isFileExist(realName);
		cocos2d::FileUtils::getInstance()->setPopupNotify(tmp);
